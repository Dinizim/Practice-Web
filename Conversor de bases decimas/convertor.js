...
/* ...Função para o tipo de dado ser apenas 0 e 1 (binario), de acordo com a tabela de keycode */
    function type_binario(e){
        //adicionar o event de clicar na teclas do teclado 
        var tecla=(window.event)?event.keyCode:e.which; //nao sei oque é isso ( ?event.keyCode:e.which )
        if ((tecla == 48 /*48 = 0 */|| tecla == 49/*49 = 1 */ )){return true; //ainda nao entendi o return
        }else{  
              return false;
        }
    }
/* ...Função para o tipo de dado ser apenas 0 a 9 (decimais), de acordo com a tabela de keycode */
    function type_decimal(e){
        var tecla=(window.event)?event.keyCode:e.which
        if ((tecla >= 48 && tecla <= 57 /*48 a 57 = 0 a 9*/)){
            return true;
        }else{
            return false;
        }
...
/* ...Função para o tipo de dado ser octal: 01234567, de acordo com a tabela de keycode */
    function type_decimal(e){
        var tecla=(window.event)?event.keyCode:e.which
        if ((tecla >= 48 && tecla <= 55 /*48 a 55 = 0 a 7*/)){
            return true;
        }else{
            return false;
        }}
/* ...Função p/ hexadecimal: 0123456789 e abcdef e ABCDEF, de acordo com a tabela de keycode */
    function type_hexadecimal(e){
        var tecla=(window.event)?event.keyCode:e.which;
        if((tecla >=48 && tecla <=57 || tecla >=65 && tecla <=70|| tecla >=97 && tecla <=102 /* ? (LER ISSO DEPOIS )*/)) return true;
        else{
        return false;
        }
    }
...
/****** FIM - Condições para habilitar e desabilitar os inputs de inserção ******/
